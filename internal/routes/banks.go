...
func GenerateBankRoutes(mainRouter *chi.Mux, service services.Service) {
	mainRouter.With(middlewares.AdminOnly).Route("/banks", func(router chi.Router) {
		router.Get("/", func(w http.ResponseWriter, r *http.Request) {
			service.ListBanksWithSortFilterPagination(
		...
		router.Get("/{id}", func(w http.ResponseWriter, r *http.Request) {
			...
			utils.ObjectFromQueryToResponse(service.GetBank, r, w, stringId)
		})

		router.Post("/", func(w http.ResponseWriter, r *http.Request) {
			bank, err := utils.DecodeBody[services.Bank](r, w)
			if err != nil {
		...
		router.Delete("/{id}", func(w http.ResponseWriter, r *http.Request) {
			...
			err := service.DeleteBank(id, r.Context())
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
			}
		})
